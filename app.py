...
app = Flask(__name__)
# Use the env var named FLASK_SECRET for Flask's SECRET_KEY
app.config['SECRET_KEY'] = os.getenv('FLASK_SECRET', 'change-me')

# Use a persistent DB path on Render Free. This file will sit on the ephemeral disk
# but survives during the life of the instance. (We don't recreate tables if present.)
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('SQLALCHEMY_DATABASE_URI', 'sqlite:////data/mbu.db')
...
def _valid_fernet_key(s: str) -> bool:
    try:
        # Must be 32 url-safe base64-encoded bytes (44-char string)
        b = (s or "").encode()
        return len(b) >= 44
    except Exception:
        return False
...
# -------------------------
# Scheduler & bootstrap (Flask 3-safe)
# -------------------------
scheduler = None
_bootstrapped = False
...
def run_minutely():
    ...
    # All DB work must be in an app context for scheduler jobs
...
def bootstrap_once():
    ...
    if _bootstrapped:
        return
    with app.app_context():
        # Create tables if not present; safe to call repeatedly
        db.create_all()
    start_scheduler()
    _bootstrapped = True

# Flask 3 removed before_first_request; use before_request to guard once.
@app.before_request
...
def signup():
    ...
    return render_template('signup.html', year=datetime.utcnow().year)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not u or not bcrypt.verify(pw, u.pw_hash):
            ...
            login_user(u)
            return redirect(url_for('dashboard'))
    return render_template('login.html', error=error, year=datetime.utcnow().year)
...
@app.route('/thank-you')
def thank_you():
    return render_template('thank.html', year=datetime.utcnow().year)
...
# -------------------------
# Trading helpers (with safe fallback broker)
# -------------------------
def choose_broker(user):
    ...
    # Fallback simple broker (prevents crashes if broker modules are missing)
    class SimpleBroker:
        name = 'demo'
        def get_price(self, _symbol): return 100.0
        def place_market_order(self, symbol, side, qty): return {'ok': True, 'symbol': symbol, 'side': side, 'qty': qty}
    return SimpleBroker()
