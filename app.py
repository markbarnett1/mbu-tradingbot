...
# -------------------------------------------------
# Load .env values (Render also injects env vars)
# -------------------------------------------------
load_dotenv()

# -------------------------------------------------
# Flask config (single place)
# -------------------------------------------------
app = Flask(__name__)

# IMPORTANT: keep this secret stable so logins (cookies) remain valid
app.config['SECRET_KEY'] = os.getenv('FLASK_SECRET', 'change-me')

# Use Render’s persistent writable path for SQLite
os.makedirs('/data', exist_ok=True)
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv(
    'SQLALCHEMY_DATABASE_URI',
    'sqlite:////data/mbu.db'  # <-- survives restarts
)
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Session cookie defaults (safe)
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
app.config['SESSION_COOKIE_SECURE'] = True  # we’re on https
...
# -------------------------------------------------
# Scheduler (Flask 3.x compatible — no before_first_request)
# -------------------------------------------------
scheduler = None
_bootstrapped = False
...
def save_broker():
    ...
    u.symbol = (request.form.get('symbol','AAPL') or 'AAPL').strip()
    try:
...
def execute_trade_safe(u):
    ...
    try:
        # Idempotency: don't exceed trades_per_day
        now_utc = datetime.now(tz=timezone.utc)
...
@app.route('/wake')
def wake():
    # Lightweight endpoint for Render Cron or GH Actions to ping.
